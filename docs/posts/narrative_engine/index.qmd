...
Here is a demo of this system with the proofread dialog for Chaointe Doran.
...
- TextNode: Displays dialog.
- ChoiceNode: Allows the player to make a narrative choice while keeping track of the selections for future use.
- AddCharacterNode: Adds a character to the narrative scene showing them standing behind the dialog.
...
- AddBackgroundNode: Sets a background color to override the location the scene takes place in. This can be extended to Textures later but ther isn't time to make bespoke drawings for scenes.
...
Conditions are used to limit when a scene can be activated and are easily extendible. They currently include:
