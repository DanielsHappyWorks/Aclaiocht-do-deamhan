...
void CharacterSelectScene::draw()
{
    ...
    if (!selectedGender)
        DrawRectangle(GetScreenWidth() / 3 - maleBody.width / 2 - 25, 85, maleBody.width + 50, maleBody.height + 30, LIGHTGRAY);
    DrawTextureEx(maleBody, (Vector2){GetScreenWidth() / 3 - maleBody.width / 2, 100}, 0.0f, 1.0f, WHITE);

    if (selectedGender)
        DrawRectangle(GetScreenWidth() / 3 * 2 - femaleBody.width / 2 - 25, 85, femaleBody.width + 50, femaleBody.height + 30, LIGHTGRAY);
    DrawTextureEx(femaleBody, (Vector2){GetScreenWidth() / 3 * 2 - femaleBody.width / 2, 100}, 0.0f, 1.0f, WHITE);

    if (GuiToggleSlider((Rectangle){GetScreenWidth() / 2 - 60, 330, 120, 30}, "Male;Female", &toggleSliderActive) == true)
    ...
    if (GuiTextBox((Rectangle){GetScreenWidth() / 2 - 60, 370, 120, 30}, name, 25, textBoxEditMode))
        textBoxEditMode = !textBoxEditMode;

    if (GuiButton((Rectangle){GetScreenWidth() / 2 - 60, 410, 120, 30}, "Play Game"))
